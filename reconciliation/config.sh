...
wikibase_main_page = 'https://${WIKIBASE_ADDRESS}/wiki/Main_Page'
...
import re
q_re = re.compile(r'(<?https?://${WIKIBASE_ADDRESS}/(entity|wiki)/)?(Q[0-9]+)>?')
q_re_group_id = 3
p_re = re.compile(r'(<?https?://${WIKIBASE_ADDRESS}/(entity/|wiki/Property:))?(P[0-9]+)>?')
p_re_group_id = 3
...
identifier_space = 'https://${WIKIBASE_ADDRESS}/entity/'
schema_space = 'https://${WIKIBASE_ADDRESS}/prop/direct/'
...
qid_url_pattern = 'https://${WIKIBASE_ADDRESS}/wiki/Item:{{id}}'
